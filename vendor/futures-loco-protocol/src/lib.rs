...
impl<T: AsyncWrite> LocoClient<T> {
    ...
    pub fn write(mut self: Pin<&mut Self>, method: Method, data: &[u8]) -> u32 {
        let id = {
    ...
    /// Queue a command with an explicit header.
    ///
    /// This is used for protocol responses that must reuse the packet id sent
    /// by the peer. It does not advance the id used by ordinary requests.
    pub fn write_command(self: Pin<&mut Self>, header: Header, data: &[u8]) {
        self.project().sink.send(Command { header, data });
    }
